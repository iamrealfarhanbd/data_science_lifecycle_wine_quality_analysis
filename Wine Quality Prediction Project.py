...
# data = data.drop_duplicates() # The original code already has this commented out, keeping it consistent
# -------------------------
...
plt.savefig(os.path.join(results_dir, 'correlation_heatmap.png')) # Save plot
...
plt.savefig(os.path.join(results_dir, 'feature_boxplots.png')) # Save plot
plt.close() # Close plot to free memory

# Pairplot for visualizing pairwise relationships
sns.pairplot(data)
plt.suptitle("Pairplot for Wine Quality Data", y=1.02) # Adjust suptitle position for better visibility
plt.savefig(os.path.join(results_dir, 'pairplot.png')) # Save plot
plt.close() # Close plot to free memory
...
rmse_best = np.sqrt(mean_squared_error(y_test, y_pred_best)) # RMSE is sqrt of MSE
...
plt.plot([y_test.min(), y_test.max()], [y_test.min(), y_test.max()], 'r--', lw=2) # A perfect prediction line
plt.xlabel('Actual Wine Quality')
plt.ylabel('Predicted Wine Quality')
plt.title('Actual vs. Predicted Wine Quality')
plt.savefig(os.path.join(results_dir, 'actual_vs_predicted_quality.png')) # Save plot
plt.close() # Close plot to free memory
...
model_filename = os.path.join(results_dir, 'best_rf_model.pkl') # Save model in results directory
